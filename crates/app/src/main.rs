...
use oxidize_injection::Registry;
...
async fn main() -> anyhow::Result<()> {
    ...
    let result = match cli.command {
        Commands::HttpServer { port } => {
            ...
            run_http_server(port, registry.into_http_state()).await
        }
        Commands::GrpcServer { port } => {
            let registry = Registry::new(&env.database_url).await?;
            run_grpc_server(port, registry.tenant_interactor, registry.staff_interactor).await
        }
        Commands::Migrate => {
